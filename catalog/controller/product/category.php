...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			//   }
				...
				$data['categories'][] = array(
					...
					'img' => $this->model_tool_image->resize($result['image'], 93, 62),
		...
		//
				...
				$data['products'][] = array(
					...
					'description' => utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 400) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
			...
			$data['continue'] = $this->url->link('common/home');
				...
				// Build ajax URL for loadMore (without page param)
			$ajax_url = '';

			if (isset($this->request->get['filter'])) {
				$ajax_url .= '&filter=' . $this->request->get['filter'];
			}

			if (isset($this->request->get['sort'])) {
				$ajax_url .= '&sort=' . $this->request->get['sort'];
			}

			if (isset($this->request->get['order'])) {
				$ajax_url .= '&order=' . $this->request->get['order'];
			}

			if (isset($this->request->get['limit'])) {
				$ajax_url .= '&limit=' . $this->request->get['limit'];
			}

			if (isset($this->request->get['sold'])) {
				$ajax_url .= '&sold=' . $this->request->get['sold'];
			}

			$data['products_total'] = $product_total;
			$data['products_per_page'] = $limit;
			$data['ajax_url'] = $this->url->link('product/category/loadMore', 'path=' . $this->request->get['path'] . $ajax_url);
	...
	public function loadMore() {
		...
		$html = $this->load->view('product/category_product', $data);

		$total_loaded = $page * $limit;
		$hasMore = $total_loaded < $product_total;
